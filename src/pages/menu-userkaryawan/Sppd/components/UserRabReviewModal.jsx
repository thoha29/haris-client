...
const UserRabReviewModal = ({ show, onClose, rab, onApproveRab, onRequestRevisi, onCancelSppd }) => {
  ...
  const grandTotalFinal = rab.grand_total_final || details.reduce((acc, curr) => {
    const qFinal = curr.jumlah_final !== null && curr.jumlah_final !== undefined
      ? Number(curr.jumlah_final)
      : (Number(curr.jumlah) || 1);
    const hFinal = curr.harga_satuan_final !== null && curr.harga_satuan_final !== undefined
      ? parseFloat(curr.harga_satuan_final)
      : (parseFloat(curr.harga_satuan) || 0);
    const tFinal = curr.total_final !== null && curr.total_final !== undefined
  ...
  const isRejected = rab.status === 'rejected' || rab.status === 'rejected_hrd';
  const isRevisiAtasan = rab.status === 'revisi_atasan';
  const isPendingAtasan = rab.status === 'pending_atasan';
  const isPendingHrd = rab.status === 'pending_hrd';
  const canCancel = rab.status !== 'cancelled' && rab.status !== 'rejected' && rab.status !== 'rejected_hrd';
            ...
            <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '8px 12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
                <span style={{
                  ...
                  {isApproved ? 'Disetujui Final' : isRejected ? 'Ditolak' : isRevisiAtasan ? 'Perlu Revisi (Atasan)' : isPendingHrd ? 'Menunggu Persetujuan Final (HRD)' : 'Menunggu Review Atasan'}
                </span>
                {selisihGrandTotal !== 0 && (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  ...
                  <tbody>
                    {details.map((item, idx) => {
                      ...
                      const qFinal = item.jumlah_final !== null && item.jumlah_final !== undefined
                        ? Number(item.jumlah_final)
                        : qAtasan;
                      const pFinal = item.harga_satuan_final !== null && item.harga_satuan_final !== undefined
                        ? parseFloat(item.harga_satuan_final)
                        : pAtasan;
                      const totFinal = item.total_final !== null && item.total_final !== undefined
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {isPendingAtasan && onApproveRab && (
              <button
                ...
                onClick={() => onApproveRab(rab.id)}
              >
                <i className="bi bi-check-circle me-1"></i>
                Setujui RAB & Teruskan ke HRD
              </button>
            )}

            {isPendingAtasan && onRequestRevisi && (
              <button
                type="button"
                style={{
                ...
                onClick={() => onRequestRevisi(rab.id)}
              >
            ...
            {canCancel && onCancelSppd && (
              <button
                ...
                onClick={() => onCancelSppd(rab.id_sppd)}
              >
